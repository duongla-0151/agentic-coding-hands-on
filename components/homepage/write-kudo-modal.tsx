...
export function WriteKudoModal({ onClose }: { onClose: () => void }) {
  ...
  const contentText = (() => { if (typeof document === "undefined") return ""; const t = document.createElement("div"); t.innerHTML = content; return t.textContent?.trim() ?? ""; })();
  const canSubmit = !!recipientId && !!badge && contentText.length > 0 && hashtags.length > 0;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.6)" }}
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div role="dialog" aria-modal="true" aria-label="Viết Kudos"
        style={{ width: "min(752px, 95vw)", maxHeight: "90vh", overflowY: "auto",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, position: "relative" }}>
          <span style={{ fontFamily: FONT, fontSize: 22, fontWeight: 700, color: "#00101A", whiteSpace: "nowrap" }}>
            Người nhận<span style={{ color: "#D32F2F" }}>*</span>
          </span>
          <div style={{ flex: 1, position: "relative" }}>
            <input type="text" placeholder="Tìm kiếm" value={recipientName}
            ...
            {showDrop && (
              <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 10,
                ...
                {users.map((u) => (
                  <div key={u.id} onMouseDown={() => { setRecipientId(u.id); setRecipientName(u.name); setShowDrop(false); }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <input type="text" placeholder="Dành tặng một danh hiệu cho đồng đội"
              value={badge} onChange={(e) => setBadge(e.target.value)}
              style={{ ...INPUT, flex: 1, width: "auto" }} />
          </div>
          <p style={{ fontFamily: FONT, fontSize: 12, color: "rgba(0,16,26,0.5)", margin: 0, paddingLeft: 4 }}>
            Ví dụ: Người truyền động lực cho tôi. Đây sẽ làm tiêu đề Kudos của bạn.
          </p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          {/* D – Rich editor */}
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <KudoRichEditor onChange={setContent} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontFamily: FONT, fontSize: 11, color: "#998C5F" }}>Tối đa 5</span>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
              {hashtags.map((tag) => (
                <span key={tag} style={{ fontFamily: FONT, fontSize: 13, color: "#00101A",
                  ...
                  #{tag}
                  <button type="button" onClick={() => setHashtags((p) => p.filter((t) => t !== tag))}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#998C5F", padding: 0, lineHeight: 1, fontSize: 16 }}>×</button>
                </span>
              ))}
              {hashtags.length < 5 && (
                <input type="text" placeholder="+ Hashtag" value={tagInput}
                  onChange={(e) => setTagInput(e.target.value)}
                  onKeyDown={(e) => { if (e.key === "Enter" || e.key === ",") { e.preventDefault(); addTag(); } }}
                  onBlur={addTag}
                  style={{ fontFamily: FONT, fontSize: 13, color: "#00101A",
                    border: "1px solid #998C5F", borderRadius: 999, padding: "4px 12px",
          ...
          {/* F – Images */}
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <span style={{ fontFamily: FONT, fontSize: 14, fontWeight: 700, color: "#00101A" }}>Image</span>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
              {previews.map((src, i) => (
                <div key={i} style={{ position: "relative", width: 80, height: 80 }}>
                  ...
                  <button type="button" onClick={() => removeImage(i)}
                    style={{ position: "absolute", top: -6, right: -6, width: 20, height: 20,
                      ...
                      color: "#fff", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
                </div>
              ))}
              {images.length < 5 && (
                <>
                  <button type="button" onClick={() => fileRef.current?.click()}
                    style={{ fontFamily: FONT, fontSize: 12, color: "#00101A",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {isAnonymous && (
            <input type="text" placeholder="Tên hiển thị (tùy chọn)" value={anonName}
              onChange={(e) => setAnonName(e.target.value)} style={INPUT} />
          )}
        </div>
        ...
        <div style={{ display: "flex", gap: 24 }}>
          <button type="button" onClick={onClose}
            style={{ fontFamily: FONT, fontSize: 14, fontWeight: 700, color: "#00101A",
